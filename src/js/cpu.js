...
*/
    ...
    let s = opcodes[this.Read(this.PC)].opsstr + " ";
...
MOS6502.prototype.MaskableInterrupt = function () {
    //DebugMessage("maskable interrupt to " + (this.Read(0xFFFE) | (this.Read(0xFFFF) << 8)));
...
MOS6502.prototype.NonMaskableInterrupt = function () {
    //DebugMessage("nonmaskable interrupt");
    this.Write(256 | this.SP, ((this.PC) >> 8) & 0xFF);
    this.SP = (this.SP - 1) & 0xFF;
    this.Write(256 | this.SP, (this.PC) & 0xFF);
    this.SP = (this.SP - 1) & 0xFF;
...
MOS6502.prototype.Step = function (steps) {
    let x = 0x0;
    let address = 0x0;
    let stack = 0x0; // used by PLP and RTI
    let help = 0x0, help2 = 0x0; //used in some operations like adc and cmp
    let count = 0;
    let command = 0;
    while (steps--) {
        command = this.Read(this.PC);
        //command = this.PC<0xA000?this.mem[this.PC]:this.Read(this.PC);
        let op = this.op[command];
        let address_mode = (op >> 8) & 0xFF;
        let code = op & 0xFF;
        count += (op >> 16) & 0xFF;
