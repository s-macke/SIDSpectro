...
let noiseLSB = new Array(NOISETABLESIZE);
let noiseMSB = new Array(NOISETABLESIZE)
let noiseMID = new Array(NOISETABLESIZE);
...
let attackrate = [2, 8, 16, 24, 38, 56, 68, 80, 100, 250, 500, 800, 1000, 3000, 5000, 8000];
let decayrate = [6, 24, 48, 72, 114, 168, 204, 240, 300, 750, 1500, 2400, 3000, 9000, 15000, 24000];
let releaserate = [6, 24, 48, 72, 114, 168, 204, 240, 300, 750, 1500, 2400, 3000, 9000, 15000, 24000];
...
let phattack = 0, phdecay = 1, phsustain = 2, phrelease = 3, phidle = 4; // phase
let gopen = 0, gclosed = 1; // gate
...
*/
    ...
    let phase = this.phase - Math.floor(this.phase);
    let oldphase = Math.floor(this.phase);
    this.phase += this.frequency / samples;
    let newphase = Math.floor(this.phase);
    ...
    switch (this.waveform) {
        ...
        case wvpulsetriangle:
            if (phase < this.pulsewidth) this.oscamplitude = 0.; else if (phase < 0.5) this.oscamplitude = phase * 2.; else this.oscamplitude = 1. - (phase - 0.5) * 2.;
            break;
...
SID6581.prototype.Write = function (index, value) {
    ...
    let oldvalue = this.regs[index];
    let i = Math.floor(index / 7); // which channel
    ...
    switch (index) {
        ...
        case 17:
            ...
            let f = this.regs[0 + i * 7] | (this.regs[1 + i * 7] << 8);
        ...
        case 18:
            ...
            if (i === 1) {
                this.Write(7, this.regs[7]);
                this.Write(8, this.regs[8]);
            }
            if (i === 2) {
                this.Write(14, this.regs[14]);
                this.Write(15, this.regs[15]);
            }
            /*
    ...
    */
            if ((value & 1) === 0) this.channel[i].gate = gclosed; else if ((oldvalue & 1) === 0) {
